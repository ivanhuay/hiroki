...
class Controller {
    constructor(model, config) {
        ...
        this.config = config || {shareQueryEnabled: true};
    ...
    request() {
        ...
        if(arguments.length === 2) {
            ...
            methods.forEach((method) => {
                const path = ['get', 'put'].includes(method) ? `/${this.routeName}/:id?` : `/${this.routeName}/`;
    ...
    _initializeShareResponse(req) {
        if(!req.hiroki || !req.hiroki.share) {
            req.hiroki = {share:{}};
            req.hirokiShare = true;
        }
    }
    queryGet(query) {
        if(query.count) {
